...
export function findAllFullPathByProcessName(processes: Process[], targetProcessName: string) {
  ...
  // Retrouver les chemins descendant 
  ...
  // retourne un tableau de chaine de caractère listant tous les chemins des processus contenant le processus cible
...
/**
 * Function that returns an array of parent process of a given target process
 * @param processes : array of process
 * @param targetProcess : process target
 * @returns : list of processes whith asc order
 */
function findParentPathByProcess(processes: Process[], targetProcess: Process) {
  ...
  // J'utilise `const` car sa référence ne changera jamais, j'utilise par la suite `unshift` pour ajouter les éléments au tableau
  const parentProcess: Process[] = [];

  // ppidToFind est une variable qui est l'id du parent à trouver dans les process, sa valeur changera au fur et à mesure de la progression dans l'arbre
  let ppidToFind: number = targetProcess.ppid;

  // fonction qui permet de renvoyer vrai ou faux si l'id du process en paramètre corresponds à ppidToFind
